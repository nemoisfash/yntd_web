...
<#macro formRadioButtons path options separator attributes="">
    ...
    <input type="radio" id="${id}" name="${status.expression}" value="${value?html}"<#if stringStatusValue == value> checked="checked"</#if><@closeTag/>${separator}${options[value]?html}
    </label>
    </#list>
</#macro>
...
-->
<#macro closeTag>
    <#if xhtmlCompliant?exists && xhtmlCompliant>/><#else>></#if><#t/>
</#macro>
